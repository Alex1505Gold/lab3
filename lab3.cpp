...
int main()
{
    ...
    std::string str3;
    std::string strH = "TMP9";
    std::cin.ignore(10, '\n');
    std::getline(std::cin, str3);
    std::string strc = "can";
    while (str2.find(strc) != std::string::npos) {
        auto pos = str2.find(strc);
        std::cout <<pos << " " << str2[pos + strc.size()] << std::endl;
        if ((str2[pos + strc.size()] == ' ') || (str2[pos + strc.size()] == '?')) {
            str2.erase(pos, strc.size());
            str2.insert(pos, str3);
        }
        else {
            str2.erase(pos, strc.size());
            str2.insert(pos, strH);
        }
    }
    while (str2.find(strH) != std::string::npos) {
        auto pos = str2.find(strH);
        //std::cout << pos << " " << str2[pos + strH.size()] << std::endl;
            str2.erase(pos, strH.size());
            str2.insert(pos, strc);
    }
    strc = "Can";
